...
def handle_acquire_lock(args: dict) -> dict[str, Any]:
    ...
    if not conversation_id or not user_id:
        raise ValueError("conversationId and userId are required.")

    if operation_type not in ("edit", "summarize"):
        raise ValueError("operationType must be 'edit' or 'summarize'.")
        ...
        if isinstance(existing_ttl, (int, float, Decimal)) and int(existing_ttl) > now:
            ...
            if existing_op in ("edit", "summarize"):
                ...
                if existing_user != user_id:
                    raise ValueError(
                        f"Currently locked by another user ({existing_user})."
                    )
...
def handle_release_lock(args: dict) -> dict[str, Any]:
    ...
    if not deleted:
        raise ValueError("No lock found for this user.")
